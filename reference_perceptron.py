...
def test_svm(svm, data):
    predicted = svm.predict([xi for xi, yi in data])
    errors = sum(predicted[i] * data[i][1] <= 0. for i in range(len(predicted)))
    print "Error rate: {}".format(errors / float(len(data)))
...
if __name__ == "__main__":
    ...
    print "\nSVM..."
    x = [xi for xi, yi in train_data]
    y = [yi for xi, yi in train_data]
